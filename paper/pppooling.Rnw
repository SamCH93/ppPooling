...
Vancomycin on \textit{Clostridium difficile}-associated diarrhoea in adults
(Table~\ref{tab:data}).

\begin{table}[!htb]
  \centering
  \caption{Historical and current data on the comparison between Fidaxcomicin and
  Vancomycin with respect to their effect on \textit{Clostridium
...
\begin{tabular}{cccc}
  \toprule
  Study & Risk (Fidaxomicin) & Risk (Vancomycin) & Risk ratio (95\% CI) \\
  \midrule
...
\begin{align*}
  ...
  = \frac{\Nor(\that \given \theta, \sigma^{2}) \Nor(\theta \given \that_0, \sigma^2_0 / \alpha) \Be(\alpha \given p, q)}{
...
\begin{figure}[!htb]
<< "figure-analysis-data", fig.height = 3.5, fig.width = 8 >>=
...
${}_2F_1(a, b, c; z) = \{\int_0^1 t^{b-1}(1-t)^{c - b -1} (1 - tz)^{-a} \text{d}t\}/\mathrm{B}(b, c - b)$
with $\mathrm{B}(x, y)$ the beta function \citep[section
15.3.1]{Abramowitz1964}, \ie{}
\begin{align}
...
representation of the hypergeometric function \citep[section
...
function
\mbox{$M(a, b, z) = \{\int_0^1 \exp(zt) t^{a-1}(1-t)^{b-a-1} \text{d}t\}/\mathrm{B}(b - a, a)$}
\citep[section 13.2.1]{Abramowitz1964} so that the marginal posterior is given
by
\begin{align}
...
\citep[section 13.1.2]{Abramowitz1964}.

\begin{figure}[!htb]
<< "figure-limiting-posterior", fig.height = 3.2, fig.width = 8 >>=
...
\label{sec:binomial}
So far we assumed a normal likelihood and a univariate parameter,
...
\begin{figure}[!htb]
<< "figure-binomial", fig.height = 3.2, fig.width = 8 >>=
...
which give most of their mass to larger values of $\alpha$, thereby shifting the
