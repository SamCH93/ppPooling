...
amounts of data \citep[section 6.6]{Held2020d}. Intuition would thus suggest
that the marginal posterior of $\alpha$ should become increasingly concentrated
at $\alpha = 1$ when there is no conflict between the data sets, and their
sample size increases. Here, we show that this is not the case, at least not in
the typical situation when $\alpha$ is assigned a beta prior distribution and
the data have normal or binomial likelihood. Instead, there is is a limiting
posterior distribution which is hardly different from the prior, implying that a
complete pooling is impossible.
